...
def create_enhanced_overlaps_analysis():
    ...
    # Add comparable provider within same label (Provider C)
    enhanced_analysis['Provider_C_PIN'] = enhanced_analysis['outlier_label'].map(
        lambda x: closest_providers[x]['pin']
    )
    enhanced_analysis['Provider_C_Name'] = enhanced_analysis['outlier_label'].map(
        lambda x: closest_providers[x]['name']
    )
    ...
    # Step 2b: Calculate Provider A to Provider C similarity (same label comparison)
    print("Step 2b: Calculating Provider A to Provider C similarities...")
    
    def calculate_provider_c_similarity(row):
        provider_a_idx = df[df['PIN'] == row['outlier_pin']].index[0]
        provider_c_idx = df[df['PIN'] == row['Provider_C_PIN']].index[0]
        
        emb_a = embeddings[provider_a_idx - df.index[0]]
        emb_c = embeddings[provider_c_idx - df.index[0]]
        
        similarity = cosine_similarity([emb_a], [emb_c])[0][0]
        return similarity
    
    enhanced_analysis['Provider_A_to_C_Similarity'] = enhanced_analysis.apply(
        calculate_provider_c_similarity, axis=1
    )
    ...
    base_cols = [
        ...
        'Provider_C_PIN', 'Provider_C_Name', 'Provider_A_to_C_Similarity',
...
print(f"\nSample of Provider similarities:")
print(enhanced_overlaps_analysis[['PIN_name', 'outlier_label', 'Provider_B_Name', 'overlap_label', 'Provider_A_to_B_Similarity', 'Provider_C_Name', 'Provider_A_to_C_Similarity']].head())
...
print("- Provider_A_to_C_Similarity shows how similar the outlier is to the typical provider in their own label")
