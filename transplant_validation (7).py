...
procedure_data = amt_smry_df[amt_smry_df['code_type'] == 'Procedure'].groupby(['code', 'PIN'])['claims'].sum()
procedure_claims = procedure_data.groupby('code').sum().to_dict()
procedure_claims_by_pin = {code: grp.to_dict() for code, grp in procedure_data.groupby(level=0)}
