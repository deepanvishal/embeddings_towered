...
with torch.no_grad():
    for i in range(0, n_samples, batch_size_infer):
        ...
        # Get embeddings from VAE towers
        proc_mu, _ = model.proc_tower.encode(proc_batch)
        diag_mu, _ = model.diag_tower.encode(diag_batch)
...
embeddings_df.to_parquet('provider_embeddings_classifier.parquet', index=False)
