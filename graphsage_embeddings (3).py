...
for epoch in range(epochs):
    ...
    for batch_idx in range(num_batches):
        ...
        for i in range(batch_edge_index.shape[1]):
            remapped_edges[0, i] = node_map[batch_edge_index[0, i].item()]
            remapped_edges[1, i] = node_map[batch_edge_index[1, i].item()]
        
        optimizer.zero_grad()
        
        # Forward pass on this batch
        local_node_ids = torch.arange(len(unique_nodes), device=device)
        z = model(unique_nodes.to(device), remapped_edges, batch_edge_attr)
...
with torch.no_grad():
    for start_idx in range(0, n_providers, chunk_size):
        ...
        # For inference, we need surrounding edges
        ...
        # Remap edges
        remapped = torch.zeros_like(chunk_edge_index)
        for i in range(chunk_edge_index.shape[1]):
            remapped[0, i] = node_map[chunk_edge_index[0, i].item()]
            remapped[1, i] = node_map[chunk_edge_index[1, i].item()]
        
        # Get embeddings
        z = model(all_nodes.to(device), remapped, chunk_edge_attr)
        
        # Extract only provider nodes from this chunk
        provider_indices = [node_map[i] for i in range(start_idx, end_idx)]
        chunk_embeddings = z[provider_indices].cpu().numpy()
