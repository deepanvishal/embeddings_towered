...
print("="*80)
print("GRAPHSAGE - TESTED WITH SAMPLE DATA")
print("="*80)
...
for epoch in range(epochs):
    ...
    for batch_idx in range(num_batches):
        ...
        # Get batch edges - MOVE TO DEVICE HERE
        batch_edge_index = data.edge_index[:, batch_edges].to(device)
        batch_edge_attr = data.edge_attr[batch_edges].to(device)
        
        # Get unique nodes in this batch (already on device)
        unique_nodes = torch.unique(batch_edge_index)
        num_unique = len(unique_nodes)
        
        # Create local node IDs (0, 1, 2, ..., num_unique-1)
        local_ids = torch.arange(num_unique, device=device)
        
        # Create mapping: global_id -> local_id
        node_to_local = torch.zeros(data.num_nodes, dtype=torch.long, device=device)
        ...
        # Encode with LOCAL indices but GLOBAL node IDs for embedding lookup
        z = model.encode(unique_nodes, local_edge_index, batch_edge_attr)
        
        # Decode: compute dot product for edges
...
with torch.no_grad():
    for chunk_idx in range(num_chunks):
        ...
        # Provider nodes we want embeddings for
        target_nodes = torch.arange(start_idx, end_idx)
        
        # Find edges connected to these providers
        ...
        # Get all unique nodes in this subgraph
        all_nodes = torch.unique(chunk_edge_index)
        num_nodes = len(all_nodes)
        
        # Create local IDs
        local_ids = torch.arange(num_nodes, device=device)
        ...
        # Extract embeddings for target providers only
        # Find which positions in all_nodes correspond to our target providers
