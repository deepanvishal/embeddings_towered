...
# ============================================================================
# CREATE AMT_SMRY VECTORS (COMBINED) WITH IDF WEIGHTING
# ============================================================================
...
# Calculate IDF weights for each code (boosts rare codes, penalizes common codes)
print("Calculating IDF weights for amt_smry codes...")
code_pin_counts = amt_grouped.groupby('code')['PIN'].nunique()
idf_weights = np.log(len(all_pins) / code_pin_counts)
amt_code_to_idf = idf_weights.to_dict()

print(f"  IDF range: {idf_weights.min():.4f} to {idf_weights.max():.4f}")
print(f"  Mean IDF: {idf_weights.mean():.4f}")

# Apply log1p transformation and IDF weighting
...
idf_values = amt_grouped['code'].map(amt_code_to_idf).values.astype(np.float32)
values = values * idf_values  # Apply IDF weighting
...
# ============================================================================
# CREATE PROCEDURE VECTORS (SEPARATE) WITH IDF WEIGHTING
# ============================================================================
...
# Calculate IDF weights for procedure codes
print("Calculating IDF weights for procedure codes...")
proc_pin_counts = proc_grouped.groupby('code')['PIN'].nunique()
proc_idf_weights = np.log(len(all_pins) / proc_pin_counts)
proc_code_to_idf = proc_idf_weights.to_dict()

print(f"  IDF range: {proc_idf_weights.min():.4f} to {proc_idf_weights.max():.4f}")
print(f"  Mean IDF: {proc_idf_weights.mean():.4f}")
...
idf_values = proc_grouped['code'].map(proc_code_to_idf).values.astype(np.float32)
values = values * idf_values  # Apply IDF weighting
...
# ============================================================================
# CREATE DIAGNOSIS VECTORS (SEPARATE) WITH IDF WEIGHTING
# ============================================================================
...
# Calculate IDF weights for diagnosis codes
print("Calculating IDF weights for diagnosis codes...")
diag_pin_counts = diag_grouped.groupby('code')['PIN'].nunique()
diag_idf_weights = np.log(len(all_pins) / diag_pin_counts)
diag_code_to_idf = diag_idf_weights.to_dict()

print(f"  IDF range: {diag_idf_weights.min():.4f} to {diag_idf_weights.max():.4f}")
print(f"  Mean IDF: {diag_idf_weights.mean():.4f}")
...
idf_values = diag_grouped['code'].map(diag_code_to_idf).values.astype(np.float32)
values = values * idf_values  # Apply IDF weighting
