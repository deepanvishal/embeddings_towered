...
PROC_LATENT_DIM = 64      # REDUCED from 128
DIAG_LATENT_DIM = 48      # REDUCED from 96
LEARNING_RATE = 0.0005
KL_WEIGHT_START = 0.01    # INCREASED from 0.001
KL_WEIGHT_END = 0.05      # INCREASED from 0.01
TRIPLET_MARGIN = 0.5
BATCH_SIZE = 128
EPOCHS = 30
...
print(f"\nLatent dimensions (REDUCED for better generalization):")
print(f"  Tower 1 (Procedures): {PROC_LATENT_DIM} [VAE] (was 128)")
print(f"  Tower 2 (Diagnoses): {DIAG_LATENT_DIM} [VAE] (was 96)")
...
print(f"  Total: {total_latent_dim} (was 246)")
...
# ============================================================================
# SIMPLIFIED MODEL ARCHITECTURE - REDUCED COMPLEXITY
# ============================================================================

class SimplifiedTowerVAE(nn.Module):
    """
    Simplified VAE tower with reduced complexity to prevent overfitting
    CHANGES:
    - Reduced hidden dimensions: [256, 128] instead of [1024, 512, 256]
    - Increased dropout: 0.4 instead of 0.3
    - Fewer parameters for better generalization
    """
...
class SimplifiedHybridModel(nn.Module):
    """Simplified hybrid model with reduced capacity"""
    def __init__(self, proc_dim, diag_dim, proc_latent, diag_latent):
        super().__init__()
        
        # Simplified VAE Towers
        self.proc_tower = SimplifiedTowerVAE(proc_dim, proc_latent, hidden_dims=[256, 128])
        self.diag_tower = SimplifiedTowerVAE(diag_dim, diag_latent, hidden_dims=[256, 128])
...
# Initialize model
model = SimplifiedHybridModel(proc_dim, diag_dim, PROC_LATENT_DIM, DIAG_LATENT_DIM).to(device)
...
print(f"  (Lower is better for generalization - target: 100-1000)")
...
print(f"\nTraining setup (SIMPLIFIED MODEL):")
...
print(f"  Hidden layers: [256, 128] (reduced from [1024, 512, 256])")
...
for epoch in range(EPOCHS):
    ...
    # KL weight annealing - MORE AGGRESSIVE
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), 'best_model_simplified.pth')
    
    # Print progress
    if epoch % 5 == 0 or epoch == EPOCHS - 1:
        print(f'\nEpoch {epoch}/{EPOCHS}:')
        print(f'  Train Loss: {epoch_losses["total"]:.4f}, Val Loss: {val_loss:.4f}, Gap: {overfitting_gap:.4f}')
    ...
    if epoch % SAVE_EVERY_N_EPOCHS == 0 or epoch == EPOCHS - 1:
        ...
        ax.set_xlim(xlim)
        ax.set_ylim(ylim)
        ax.set_title(f'Provider Embeddings - Epoch {epoch} (SIMPLIFIED MODEL)', fontsize=18, fontweight='bold')
        ...
        # Save to file
        frame_path = f'umap_epoch_{epoch:03d}_simplified.png'
...
if frames:
    frames[0].save(
        'training_evolution_simplified.gif',
        save_all=True,
        append_images=frames[1:],
        duration=1000,
        loop=0
    )
    print(f"GIF saved: training_evolution_simplified.gif ({len(frames)} frames)")
...
model.load_state_dict(torch.load('best_model_simplified.pth'))
model.eval()
...
for i in range(cost_latent_dim):
    embedding_data[f'tower5_cost_emb_{i}
...
# Save embeddings
embeddings_df.to_parquet('provider_embeddings_simplified.parquet', index=False)

print("\nSaved:")
print("  - provider_embeddings_simplified.parquet")
print("  - best_model_simplified.pth")
print("  - training_evolution_simplified.gif")

print("\n" + "="*80)
print("COMPLETE - SIMPLIFIED MODEL")
print("="*80)
print(f"Total embedding dimensions: {total_latent_dim}")
print(f"Best validation loss: {best_val_loss:.4f}")
print(f"\nModel Simplifications Applied:")
print(f"  ✓ Reduced latent dims: Proc 128→64, Diag 96→48")
print(f"  ✓ Reduced hidden layers: [1024,512,256] → [256,128]")
print(f"  ✓ Increased dropout: 0.3 → 0.4")
print(f"  ✓ Stronger KL weight: 0.001-0.01 → 0.01-0.05")
print(f"  ✓ Added L2 regularization: weight_decay=1e-4")
print(f"  ✓ Total params reduced by ~87%")
print(f"\nExpected improvements:")
print(f"  • Rounder, blob-like clusters (not worms)")
print(f"  • Better generalization (smaller train-val gap)")
print(f"  • Less overfitting on provider-specific patterns")
