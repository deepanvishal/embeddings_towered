...
def create_embedding_comparison_chart(outlier_pin, typical_same_pin, typical_other_pin, outlier_name, typical_same_name, typical_other_name):
    """Create embedding comparison chart for providers A, B, C."""
    
    # Get embeddings for the three providers
    outlier_emb = embeddings[df[df['PIN'] == outlier_pin].index[0] - df.index[0]]
    typical_same_emb = embeddings[df[df['PIN'] == typical_same_pin].index[0] - df.index[0]]
    typical_other_emb = embeddings[df[df['PIN'] == typical_other_pin].index[0] - df.index[0]]
    
    # Create the plot
    plt.figure(figsize=(16, 8))
    
    x_positions = range(64)  # 64 embedding dimensions
...
def create_pca_chart(outlier_pin, typical_same_pin, typical_other_pin, outlier_label, overlap_label, outlier_name, typical_same_name, typical_other_name):
    """Create PCA visualization chart for Excel export."""
