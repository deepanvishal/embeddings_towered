...
def analyze_code_frequencies(df, pin_col='srv_prvdr_id', code_col='code', claims_col='claims'):
    """
    Analyze code frequencies to identify low-frequency outliers
    Shows distribution, thresholds, and provider impact at each threshold
    
    Args:
        df: DataFrame with three columns:
            - pin_col: Provider ID (PIN)
            - code_col: Code (procedure/diagnosis)
            - claims_col: Number of claims
    
    Returns:
        Dictionary with analysis results
    """
    
    print("Aggregating code frequencies...")
    
    # Aggregate: sum claims by code, count unique PINs per code
    code_stats = df.groupby(code_col).agg({
        claims_col: 'sum',
        pin_col: 'nunique'
    }).reset_index()
    code_stats.columns = ['code', 'total_claims', 'unique_pins']
    
    print(f"\nTotal unique codes: {len(code_stats):,}")
    print(f"Total claims: {code_stats['total_claims'].sum():,}")
    print(f"Total unique PINs in dataset: {df[pin_col].nunique():,}")
    ...
    for i, threshold in enumerate(breakpoints):
        if i == 0:
            ...
            pins_affected = df[df[code_col].isin(
                code_stats[code_stats['total_claims'] == 1]['code']
            )][pin_col].nunique()
            label = "= 1"
        else:
            ...
            pins_affected = df[df[code_col].isin(
                code_stats[(code_stats['total_claims'] > prev) & 
                          (code_stats['total_claims'] <= threshold)]['code']
            )][pin_col].nunique()
        ...
        breakdown_data.append({
            ...
            'PINs Affected': pins_affected,
            'PINs %': f"{pins_affected/df[pin_col].nunique()*100:.1f}%"
        })
    
    # Add > 100
    codes_count = (code_stats['total_claims'] > 100).sum()
    pins_affected = df[df[code_col].isin(
        code_stats[code_stats['total_claims'] > 100]['code']
    )][pin_col].nunique()
    breakdown_data.append({
        'Threshold': "> 100",
        'Codes': codes_count,
    ...
    print("\n" + "="*80)
    print("THRESHOLD IMPACT ANALYSIS - PINs FILTERED OUT")
    print("="*80)
    ...
    for name, threshold_val in threshold_options:
        ...
        codes_below = code_stats[code_stats['total_claims'] < threshold_val]['code'].tolist()
        
        # PINs that would be affected (have at least one code below threshold)
        pins_affected = df[df[code_col].isin(codes_below)][pin_col].nunique()
        
        # Codes removed
        codes_removed = len(codes_below)
        ...
        impact_data.append({
            ...
            'PINs Affected': pins_affected,
            'PINs Affected %': f"{pins_affected/df[pin_col].nunique()*100:.1f}%"
        })
...
# Usage Example:
# df = pd.DataFrame({
#     'srv_prvdr_id': ['PIN001', 'PIN001', 'PIN002', 'PIN002', 'PIN003', ...],
#     'code': ['99213', '99214', '99213', '70450', '99213', ...],
#     'claims': [10, 5, 8, 2, 15, ...]
# })
# 
# # Filter for specific code type first
# procedure_df = df[df['code_type'] == 'procedure']
# 
# # Run analysis
# results = analyze_code_frequencies(procedure_df, 
#                                   pin_col='srv_prvdr_id',
#                                   code_col='code', 
#                                   claims_col='claims')
