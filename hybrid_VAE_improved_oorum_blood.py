...
class ImprovedTowerVAE(nn.Module):
    """
    Improved VAE tower with:
    - Larger latent dimensions (128/96 instead of 64/48)
    - Better architecture for reconstruction
    """
    def __init__(self, input_dim, latent_dim, hidden_dims=[512, 256]):
        super().__init__()
        
        # Encoder - slightly larger for better capacity
        encoder_layers = []
        prev_dim = input_dim
        ...
        # No activation - predicting log-space values directly
...
class ImprovedHybridModel(nn.Module):
    """Improved hybrid model with larger capacity"""
    def __init__(self, proc_dim, diag_dim, proc_latent, diag_latent):
        super().__init__()
        
        # Improved VAE Towers with larger hidden dims
        self.proc_tower = ImprovedTowerVAE(proc_dim, proc_latent, hidden_dims=[512, 256])
        self.diag_tower = ImprovedTowerVAE(diag_dim, diag_latent, hidden_dims=[512, 256])
...
print("="*80)
print(f"\nImprovements:")
print(f"  - MSE on log-space (instead of BCE)")
print(f"  - Masked loss (focus on non-zero codes)")
print(f"  - Larger latent dims: Proc {PROC_LATENT_DIM}, Diag {DIAG_LATENT_DIM}")
print(f"  - Better hidden architecture: [512, 256]")
print(f"  - Separate loss weighting for proc/diag")
