...
# Procedure codes per PIN (ALL providers)
...
# Diagnosis codes per PIN (ALL providers)
...
# Set PIN as index for easy lookup (ALL providers)
    ...
    for rank, (rec_idx, rec_sim) in enumerate(zip(top_10_indices, top_10_sims), 1):
        ...
        if query_pin in demo_df.index:
            for col in demo_cols:
                result[f'provA_{col}'] = demo_df.loc[query_pin, col]
        else:
            for col in demo_cols:
        ...
        if rec_pin in demo_df.index:
            for col in demo_cols:
                result[f'provB_{col}'] = demo_df.loc[rec_pin, col]
        else:
            for col in demo_cols:
        ...
        if query_pin in place_df.index:
            for col in place_cols:
                result[f'provA_{col}'] = place_df.loc[query_pin, col]
        else:
            for col in place_cols:
                result[f'provA_{col}'] = np.nan
        ...
        if rec_pin in place_df.index:
            for col in place_cols:
                result[f'provB_{col}'] = place_df.loc[rec_pin, col]
        else:
            for col in place_cols:
                result[f'provB_{col}'] = np.nan
        ...
        if query_pin in cost_df.index:
            for col in cost_cols:
                result[f'provA_{col}'] = cost_df.loc[query_pin, col]
        else:
            for col in cost_cols:
                result[f'provA_{col}'] = np.nan
        ...
        if rec_pin in cost_df.index:
            for col in cost_cols:
                result[f'provB_{col}'] = cost_df.loc[rec_pin, col]
        else:
            for col in cost_cols:
                result[f'provB_{col}'] = np.nan
        ...
        if query_pin in pin_df.index:
            for col in pin_cols:
                result[f'provA_{col}'] = pin_df.loc[query_pin, col]
        else:
            for col in pin_cols:
                result[f'provA_{col}'] = np.nan
        ...
        if rec_pin in pin_df.index:
            for col in pin_cols:
                result[f'provB_{col}'] = pin_df.loc[rec_pin, col]
        else:
            for col in pin_cols:
                result[f'provB_{col}'] = np.nan
