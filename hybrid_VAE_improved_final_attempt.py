...
class HierarchicalHybridModel(nn.Module):
    ...
    def __init__(self, proc_dim, diag_dim, proc_latent, diag_latent, n_classes):
        ...
        total_latent = proc_latent + diag_latent + 5 + 4 + 11 + 2  # all tower dims
...
model = HierarchicalHybridModel(proc_dim, diag_dim, PROC_LATENT_DIM, DIAG_LATENT_DIM, n_classes).to(device)
    ...
    with torch.no_grad():
        for i in range(0, len(val_indices), BATCH_SIZE):
            ...
            if batch_labels is not None and len(batch_labels) > 0:
                logits = model.classify(combined_emb)
