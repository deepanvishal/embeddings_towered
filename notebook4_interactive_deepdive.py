...
def get_top_procedures(pin, top_n=10):
    ...
    if pin not in pin_to_procedure_data:
        return pd.DataFrame(columns=['code', 'claims', '%'])
    ...
    return df.nlargest(top_n, 'claims')[['code', 'claims', '%']]


def get_top_diagnoses(pin, top_n=10):
    """Get top diagnoses for a provider"""
...
def find_common_procedures(pin_a, pin_b, top_n=10):
    """Find common procedures between two providers"""
    proc_a = get_top_procedures(pin_a, 50)
    proc_b = get_top_procedures(pin_b, 50)
    
    if len(proc_a) == 0 or len(proc_b) == 0:
        return pd.DataFrame(columns=['code', 'claims_A', 'claims_B', '%_A', '%_B'])
    
    common = proc_a.merge(proc_b, on='code', suffixes=('_A', '_B'), how='inner')
    common['total_claims'] = common['claims_A'] + common['claims_B']
    
    return common.nlargest(top_n, 'total_claims')[['code', 'claims_A', 'claims_B', '%_A', '%_B']]


def find_common_diagnoses(pin_a, pin_b, top_n=10):
    """Find common diagnoses between two providers"""
    ...
    common = diag_a.merge(diag_b, on='code', suffixes=('_A', '_B'), how='inner')
    common['total_claims'] = common['claims_A'] + common['claims_B']
