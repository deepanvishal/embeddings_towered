...
def write_analysis_to_worksheet(ws, selected_row, analysis_data, tab_name):
    """Write complete analysis data to worksheet."""
    ...
    ws[f'A{current_row}'] = f"Selected Provider: {selected_row['outlier_name']} ({selected_row['outlier_label'].title()})"
    current_row += 1
    ws[f'A{current_row}'] = f"Overlapping with: {selected_row['overlap_label'].title()}"
    current_row += 1
    ws[f'A{current_row}'] = f"Distance to Own Centroid: {selected_row['own_distance']:.4f}"
    current_row += 1
    ws[f'A{current_row}'] = f"Distance to Other Centroid: {selected_row['other_distance']:.4f}"
    current_row += 1
    ws[f'A{current_row}'] = f"Overlap Score: {selected_row['overlap_score']:.4f}"
    current_row += 3
    
    # Within-group analysis
    ws[f'A{current_row}'] = f"WITHIN-GROUP ANALYSIS: {selected_row['outlier_label'].title()}"
    ws[f'A{current_row}'].font = Font(size=12, bold=True)
    current_row += 2
    
    # Procedures
    if len(analysis_data['procedures']['outlier']) > 0:
        ws[f'A{current_row}'] = f"Top Procedures - {selected_row['outlier_name']}"
