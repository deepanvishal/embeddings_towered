...
class SpecialtyAutoencoder(nn.Module):
    """Hierarchical autoencoder for specialty-specific procedures"""
...
print("\n" + "="*80)
print("TRAINING SPECIALTY AUTOENCODERS")
print("="*80)
...
for specialty_name, code_indices in specialty_code_indices.items():
    ...
    # Get hospitals with this specialty
    specialty_pins = [pin for pin, label in pin_to_label.items() if label == specialty_name]
    
    if len(specialty_pins) < 10:
        print(f"\nSkipping {specialty_name}: only {len(specialty_pins)} hospitals (need at least 10)")
        continue
    
    # Get indices in the full tensor
    specialty_indices = [pin_to_idx[pin] for pin in specialty_pins]
    
    # Extract specialty-specific procedure columns
    X_specialty = proc_tensor[specialty_indices][:, code_indices]
    ...
    # Save model
    model_filename = f"autoencoder_{specialty_name.replace(' ', '_')}.pth"
...
with open('specialty_autoencoders_metadata.pkl', 'wb') as f:
    pickle.dump(metadata, f)

print(f"\nSaved metadata: specialty_autoencoders_metadata.pkl")
...
print("="*80)
print("\nNext step: Run Phase 2 to encode all hospitals")
